  ...
  methods: {
    async fetchData() {
      ...
      function addCityNameToWorkArray() {
        ...
        workArray = areaIDWithCityName.map((entry) =>
          transformSingleEntry(entry, workArray)
        );
      ...
      function addCapacityToWorkArray(dataSet) {
        let newData = dataSet.map((entry) => [
          entry.areaid,
          { capacity: entry.capacity, laadPalen: entry.chargingpointcapacity },
        ]);
        workArray = newData.map((entry) =>
          transformSingleEntry(entry, workArray)
        );
        workArray = workArray.filter((entry) => !isNull(entry));
      }
      ...
      function transformSingleEntry(entryToAdd, dataSetToBeAddedTo) {
        ...
        if (dataSetToBeAddedTo.find((entry) => entry[0] == entryToAddId)) {
          let matchedArray = dataSetToBeAddedTo.find(
            (entry) => entry[0] == entryToAddId
          );
      ...
      function addDaysToWorkArray(dataSet) {
        let allAreaIdsPerDay = Object.entries(
          (workArray = groupByKey(dataSet, "areaid"))
        );
